...
const makeRequest = async (method, path, payload, token) => {
  let device_token, customer_token, refererUrl, currency, apiHeaders;
  ...
  if (typeof window !== "undefined") {
    ...
    currency = Cookies.get("cr_currency");
    apiHeaders = {
      "device-token": device_token,
      "customer-token": token ?? customer_token,
      "referer-url": refererUrl,
    };
    if (currency) apiHeaders = { ...apiHeaders, currency };
  } else {
    ...
    currency = cookies().get("cr_currency")?.value;
    apiHeaders = {
      "device-token": device_token,
      "customer-token": token ?? customer_token,
      "referer-url": refererUrl,
